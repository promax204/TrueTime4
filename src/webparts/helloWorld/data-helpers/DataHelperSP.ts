        ...
        this.getTermGroupById(termStore, termSet.termGroupId).then((group) => { // getting group by id
          ...
          this.getTermSetById(termStore, group, termSet.id).then((set) => { // getting term set by id
            ...
            try {
              ...
              this.clientContext.load(allTerms, 'Include(Id, Name, Description, IsRoot, TermsCount, PathOfTerm, Labels, CustomProperties)');
              this.clientContext.executeQueryAsync(() => {
                // caching terms
                //console.log("allTerms", allTerms);
                //var properties = allTerms.[0].get_objectData().get_properties()["CustomProperties"];
  ...
  private buildTermsHierarchy(terms: SP.Taxonomy.ITermCollection, termSetId: string): ITerms {
    ...
    for (let i = 0, len = terms.get_count(); i < len; i++) {
      ...
      const termEntity: ITermWithTerms = {
        ...
        termSetId: termSetId,
        CustomProperties: term.get_objectData()
        //labelCollection: term.getAllLabels()//get_objectData().get_properties()["CustomProperties"] gotogoto
      };
